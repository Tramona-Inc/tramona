...
function StepperContentLayout({
  ...
  return <div className={cn('mt-2 flex flex-col gap-4 self-center', className)}>{children}</div>;
}
        ...
        {fakeLiveDeals.slice(2).map(({ minutesAgo, tramonaPrice, oldPrice }, idx) => (
          <div className="flex flex-row gap-4 rounded-lg border-2 bg-white p-4 text-black" key={idx}>
            ...
            <div className="relative grid lg:grid-cols-2">
              <span className="absolute right-[-1rem] top-[-1rem] text-xs text-[#192185]">{minutesAgo}min ago</span>
              <div className="flex flex-col items-center justify-center">
                <p className="font-bold">Tatiana is going to Paris 🇫🇷</p>
                <p className="text-xs">
                  Tramona Price: <span className="font-bold text-[#3843D0] lg:text-lg">{tramonaPrice}$/per night</span>{' '}
                </p>
              </div>
              <div className="flex flex-col items-center justify-center">
              ...
              <p className="text-xs text-[#3843D0]">
                {Math.round(100 * (1 - (tramonaPrice ?? 0) / (oldPrice ?? 0)))}% Off!
              </p>
            </div>
          </div>
        ))}
  ...
  return (
    <StepperContentLayout className={'items-center text-center'}>
      <p className="text-l text-center font-bold">
        We have the <span className="bg-[#FACF26] px-1">same hosts</span> that are on sites like Airbnb, VRBO, or
        Bookings.com, <br /> we just match you with one of their vacant dates.
      </p>
      <p>Its basically free money. Try it for yourself.</p>
      <p>We will never ask for a credit card or any form of payment, until its time to checkout</p>
      <Image
...
function Step3(): JSX.Element {
  return (
    <StepperContentLayout className={'text-l items-center text-center text-center font-bold'}>
      <p>We have a global feed and a personal feed.</p>
      <p>
        You can see where your friends are going, deals other people have gotten, <br /> or just find inspiration for
        yourself
      </p>
      <Image
  ...
  return (
    <StepperContentLayout className={'items-center text-center'}>
      ...
      <p className="text-l font-bold">
        We built Tramona for the sole benefit of travelers and hosts, our referral program is no different.
      </p>
      <p>
        To show this, we offer a generous <span className="bg-[#FACF26] px-1">30%</span> base revenue split with people
...
function Step5(): JSX.Element {
  return (
    <StepperContentLayout className={'items-center text-center'}>
      <p className="text-l font-bold">We want to increase the the amount people travel, its that simple.</p>
      <p className="bg-[#FACF26] px-1">Submit a request and see how it works. </p>
      <Image
