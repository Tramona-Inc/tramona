...
import { lt, and, isNotNull, isNull, inArray } from "drizzle-orm";
...
type GroupedRequests = Record<
  string,
  Record<string, Record<string, Request[]>>
>;
...
export default async function handler() {
  const unsentRequestTexts: Request[] = await db.query.requests.findMany({
    ...
    where: and(
      isNotNull(requests.messageCase),
      isNull(requests.messageSent),
      lt(requests.createdAt, subMinutes(new Date(), 3)),
    ),
  });

  const groupedByPhoneNumber = unsentRequestTexts.reduce(
    (acc: GroupedRequests, request: Request) => {
        ...
        if (userRequests && userRequests.length > 0) {
          ...
          switch (messageCase) {
            ...
            case "Some close matches":
              totalMatches = await db.query.offers
                .findMany({
