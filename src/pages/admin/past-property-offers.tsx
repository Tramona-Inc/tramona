...
function PastPropertyOfferCards({
  ...
  propertyOffers?: RouterOutputs["biddings"]["getAllAccepted"];
}) {
  ...
  return (
    <div className="space-y-2">
      {propertyOffers.map((offer) => (
        ...
        <PropertyOfferAcceptedCard key={offer.id} offer={offer} />
      ))}
    </div>
  );
}
...
export default function Page() {
  const { data: propertyOffers } = api.biddings.getAllAccepted.useQuery();
    ...
    <DashboardLayout type="admin">
      ...
      <div className="px-4 pb-64 pt-16">
        <div className="mx-auto max-w-5xl">
          ...
          <PastPropertyOfferCards propertyOffers={propertyOffers} />
        </div>
      </div>
    </DashboardLayout>
  );
