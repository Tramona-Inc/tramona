...
const createAxiosInstance = (): AxiosInstance => {
  ...
  axiosRetry(instance, {
    ...
    retryCondition: (error: AxiosError): boolean  => {
      // Retry on knowing errors 
      return error.code === "522" || error.code === 'EPROTO';
    },
  });
...
export const arizonaScraper: DirectSiteScraper = async ({
  ...
  const properties = await axiosInstance.get(url, { httpsAgent: proxyAgent })
  ...
  const propertiesWithReviews = await Promise.all(properties.map(async (p) => {
    ...
    const reviews = await axiosInstance.get(reviewUrl, { timeout: 30000, httpsAgent: proxyAgent })
      // eslint-disable-next-line @typescript-eslint/no-unsafe-return
      .then((response) => response.data)
