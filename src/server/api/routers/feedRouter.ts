...
import { generateReferralCode, getNumNights } from "@/utils/utils";
...
import { createCreationTime, createRandomDate, createUserNameAndPic, getRandomOffer, randomizeLocationAndPrice } from "@/components/activity-feed/admin/generationHelper";
...
export const feedRouter = createTRPCRouter({
    getFeed: publicProcedure
        ...
        .query(async ({ ctx, input }) => {
            ...
            const fillerRequests = await ctx.db.query.fillerRequests.findMany({
                ...
                limit: input.atLeastNumOfEntries ?? 30,
                orderBy: (fillerRequests, { desc }) => [desc(fillerRequests.entryCreationTime)],
            });
            const fillerOffers = await ctx.db.query.fillerOffers.findMany({
                ...
                limit: input.atLeastNumOfEntries ?? 30,
                orderBy: (fillerOffers, { desc }) => [desc(fillerOffers.entryCreationTime)],
            });
            const fillerBookings = await ctx.db.query.fillerBookings.findMany({
                ...
                limit: input.atLeastNumOfEntries ?? 30,
                orderBy: (fillerBookings, { desc }) => [desc(fillerBookings.entryCreationTime)],
            });
          ...
          const mergedData = [
            ...
            ...fillerRequests.map(item => ({
                ...
                madeByGroup: {
                    owner: { id: "", name: item.userName, image: item.userProfilePicUrl },
                },
                uniqueId: `fr-${item.id}`,
                type: 'request' as const,
                isFiller: true,
            ...
            ...fillerOffers.map(item => ({
                ...
                property: {
                    ...
                    originalNightlyPrice: item.originalNightlyPrice || item.property.originalNightlyPrice, // if admin didn't specify, use the property's nightly price
                },
                request: {
                    madeByGroup: {
                        owner: { id: "", name: item.userName, image: item.userProfilePicUrl },
            ...
            ...fillerBookings.map(item => ({
                ...
                property: {
                    id: item.propertyId,
                    imageUrls: item.property.imageUrls,
                    originalNightlyPrice: item.originalNightlyPrice || item.property.originalNightlyPrice,
    ...
    updateFillerRequest: roleRestrictedProcedure(["admin"])
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            await ctx.db.update(fillerRequests).set(input).where(eq(fillerRequests.id, input.id));
        }),
    ...
    updateFillerOffer: roleRestrictedProcedure(["admin"])
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            await ctx.db.update(fillerOffers).set(input).where(eq(fillerOffers.id, input.id));
        }),
    ...
    updateFillerBooking: roleRestrictedProcedure(["admin"])
        ...
        .mutation(async ({ ctx, input }) => {
            ...
            await ctx.db.update(fillerBookings).set(input).where(eq(fillerBookings.id, input.id));
        }),

    deleteFillerRequest: roleRestrictedProcedure(["admin"])
        .input(fillerRequestUpdateSchema.pick({ id: true }))
        .mutation(async ({ ctx, input }) => {
            await ctx.db.delete(fillerRequests).where(eq(fillerRequests.id, input.id));
        }),
    ...
    deleteFillerBooking: roleRestrictedProcedure(["admin"])
        ...
        .mutation(async ({ ctx, input }) => {
            await ctx.db.delete(fillerBookings).where(eq(fillerBookings.id, input.id));
        }),
    ...
    generateFillerData: roleRestrictedProcedure(["admin"])
        .mutation(async ({ ctx }) => {
            ...
            const [userData, locationAndPrices] = await Promise.all([userDataPromise, locationAndPricePromise]);
            for (let i = 0; i < NUM_OF_REQUESTS; i++) {
                counter++;
                let username = "";
                let picture = "";
                if (userData && userData[i] && i < userData.length ) {
                ...
                if(random && random.location && randomDate.checkIn && randomDate.checkOut){
                    const fillerRequest = {
                        userName: username,
            ...
            const generatedOffers = realOffers.filter(offer => offer.property.originalNightlyPrice !== null)
                .map(offer => {
                ...
                if(!originalNightlyPrice) throw new Error('originalNightlyPrice is undefined');
            
                const actualNightlyPrice = totalPrice / nights;
                let generatedNightlyPrice: number;

                if (actualNightlyPrice < 0.3 * originalNightlyPrice || actualNightlyPrice >= originalNightlyPrice) {
                    const minPrice = 0.5 * originalNightlyPrice;
                    const maxPrice = 0.9 * originalNightlyPrice;
                    generatedNightlyPrice = minPrice + (Math.random() * (maxPrice - minPrice));
                } else {
                    const minPrice = 0.8 * actualNightlyPrice;
                    const maxPrice = Math.min(1.2 * actualNightlyPrice, originalNightlyPrice);
                    generatedNightlyPrice = minPrice + (Math.random() * (maxPrice - minPrice));
                }
            ...
            for (let i = 0; i < NUM_OF_OFFERS; i++) {
                ...
                let username = "";
                let picture = "";
                if (userData && userData[counter] && counter < userData.length ) {
                    username = userData[counter]?.name ?? "";
                    picture = userData[counter]?.picture ?? "";
                }
                const randomDate = createRandomDate();
                const creationTime = createCreationTime();
                const offer =   getRandomOffer(generatedOffers);
                let numNights = 0;
                if (randomDate.checkIn && randomDate.checkOut) {
                    numNights = getNumNights(randomDate.checkIn, randomDate.checkOut);
                }
                if( offer && randomDate.checkIn && randomDate.checkOut){
                    ...
                    await ctx.db.insert(fillerOffers).values(fillerOffer);
                }
            }
            ...
            for (let i = 0; i < NUM_OF_BOOKINGS; i++) {
                ...
                const randomDate = createRandomDate();
                const creationTime = createCreationTime();
                const booking =   getRandomOffer(generatedOffers);
                let numNights = 0;
                if (randomDate.checkIn && randomDate.checkOut) {
                    numNights = getNumNights(randomDate.checkIn, randomDate.checkOut);
                }
