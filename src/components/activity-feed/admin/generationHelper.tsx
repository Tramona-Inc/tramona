...
export async function createUserNameAndPic(quantity?: number): Promise<{ name: string; picture: string; }[]>  {
    try {
        const response = await fetch('https://randomuser.me/api/?nat=us,ca,mx,gb&inc=name,picture' + (quantity ? `&results=${quantity}` : ''));
        if (!response.ok) {
...
export function createRandomDate() {
  ...
  const endDate = new Date(startDate.getTime() + (Math.floor(Math.random() * 7) + 1) * 24 * 60 * 60 * 1000); // 1-7 days after start date
...
export function createCreationTime() {
    ...
    const now = new Date();
    const pastTime = new Date(now.getTime() - Math.random() * 24 * 60 * 60 * 1000); // Random time within past 24 hours
    const randonPastTime = pastTime.toLocaleString('sv-SE', { timeZoneName: 'short' }).replace(' ', 'T').slice(0, 19).trim(); // Convert to local time, Format as "YYYY-MM-DDTHH:mm:ss(s)"
    return padHourInDatetime(randonPastTime);
}

export function randomizeLocationAndPrice(locationAndPrices: { location: string; maxTotalPrice: number; checkIn: Date; checkOut: Date}[]) {
    ...
    if ( realLocationAndPrice?.maxTotalPrice) {
        const realNightlyPrice = realLocationAndPrice?.maxTotalPrice / getNumNights(realLocationAndPrice?.checkIn, realLocationAndPrice?.checkOut) / 100;
        const randomNightlyPrice = Math.floor(realNightlyPrice * (1 + (Math.random() * 0.6 - 0.3))); // Random price within +/- 30% of the real price
...
export function getRandomOffer(offers: any[]): any {
    if (offers.length === 0) {
        throw new Error("The offers array is empty");
    }
