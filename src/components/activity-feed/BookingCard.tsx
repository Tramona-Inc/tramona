...
import CreateBookingDialog from "./admin/BookingDialog";

export default function FeedbookingCard({
  booking,
  children,
}: React.PropsWithChildren<{
  booking: BookingCardDataType;
}>) {
    ...
    <BaseCard item={booking} userName={userName} userImage={userImage}>
      ...
      <div className="w-full space-y-2"> 
        <div className="flex items-start justify-between w-full">
          ...
          {booking.isFiller && (
          <CreateBookingDialog booking={booking}>
        ...
        <div className="relative">
          ...
          >
            <CarouselContent className="-ml-2 md:-ml-4">
              {booking.property.imageUrls.slice(0, 5).map((photo, index) => (
                <CarouselItem
                  key={index}
