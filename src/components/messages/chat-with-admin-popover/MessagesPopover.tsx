  ...
  export default function MessagesPopover() {
    ...
    if (!session && typeof window !== "undefined") {
      ...
      if (!tempToken) {
        ...
        void createTempUserForGuest({ email: "temp_user@gmail.com", isGuest: true, sessionToken: tempToken });
    ...
    const { data: conversationId } =
        api.messages.getConversationsWithAdmin.useQuery({
            userId: session?.user.id ?? tempToken,
        });
    ...
    const sender = {
        name: "Tramona Info",
        image: "https://lh3.googleusercontent.com/a/ACg8ocLm7RrPPFKJd_jo8vFOMMZc_nj-u30AgO5YCx6keJbu04P0cnE=s96-c"
    }
    ...
    const handleOnSend = async (values: z.infer<typeof formSchema>) => {
      ...
      if (!session) {
        const {tempUserId, conversationId} = await createOrRetrieveConversationFromGuest({sessionToken: tempToken});
