...
function useStepper({ initialStep, steps }: useStepperProps): useStepperReturn {
  ...
  const isLastStep = React.useMemo(() => activeStep === steps.length - 1, [activeStep, steps.length]);

  const isOptionalStep = React.useMemo(() => steps[activeStep]?.optional, [steps, activeStep]);
