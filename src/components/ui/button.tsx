  ...
  'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50',
  {
    variants: {
      variant: {
        ...
        outline: 'border-2 border-primary bg-background text-primary hover:bg-primary/10 focus-visible:bg-primary/10',
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, isLoading = false, children, ...props }, ref) => {
    ...
    return (
      ...
      >
        <>
          <div className={`transition-all duration-300 ${isLoading ? 'mr-2 w-6' : 'w-0'}`}>
            <Loader2Icon className={`animate-spin transition-opacity ${isLoading ? 'opacity-100' : 'opacity-0'}`} />
          </div>
          {children}
        </>
      </Comp>
