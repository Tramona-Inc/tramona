import {
  ...
  export const useCohostOnboarding = create<CohostOnboardingState>()(
    persist(
      (set) => ({
        progress: 0,
        isEdit: false,
        ...
        resetSession: () => {
          sessionStorage.removeItem("cohost-onboarding-state");
        },
      }),
      {
        name: "cohost-onboarding-state", // name of the item in the storage (must be unique)
